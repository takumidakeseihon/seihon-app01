...
def load_user_report_history(_db_dummy, user_name, limit_count=30):
    try:
        ...
        with st.spinner("通信量を節約しながらあなたの履歴だけを取得中..."):
            ...
            if history_df.empty:
                ...
                for _, h_row in history_df.iterrows():
                    ...
                    h_mac = h_row.get('機械の調子', '未記入')
                    h_note = h_row.get('特記事項', 'なし')
                    h_hiyari = h_row.get('ヒヤリハット', 'なし')
                    
                    h_arr_disp = str(h_arr) if "なし" not in str(h_arr) else "通常"
                    h_lev_disp = str(h_lev) if "なし" not in str(h_lev) else "定時"
                    
                    st.markdown(f"**{h_date}** | 出勤: {h_arr_disp} / 退勤: {h_lev_disp} | 🔧 {h_mac}")
                    
                    if str(h_note).strip() and str(h_note) != "nan":
                        st.caption(f"💡 特記事項: {h_note}")
                    if "あり" in str(h_hiyari):
                        st.markdown(f"<div style='color:#d97706; font-size:0.8rem; margin-top:-10px; margin-bottom:10px;'>⚠️ ヒヤリハット: {h_hiyari}</div>", unsafe_allow_html=True)
